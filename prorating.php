      ...
      <?php 
                     ...
                     // session_start();
                         $service = $_GET['id'];
                              ...
                              //echo $service; 
                             $sql = "SELECT serviceprovider.providerid,serviceprovider.img_text,serviceprovider.providername,serviceprovider.visit_charge,serviceprovider.description, providerservices.serviceid,AVG(sp_rating.rating) AS 'ave rating' FROM serviceprovider,providerservices,sp_rating WHERE serviceprovider.providerid=providerservices.provider_id AND serviceprovider.providerid=sp_rating.providerid AND providerservices.serviceid=$service GROUP BY sp_rating.providerid ORDER BY AVG(sp_rating.rating) DESC ";
                               $result = $conn->query($sql);
                              while($row = mysqli_fetch_assoc($result)) {?>
          <div class="row p-2 bg-white border rounded">
              <div class="col-md-3 mt-1">
              <img src="data:images/jpg;charset=utf8;base64,<?php echo base64_encode($row['img_text']); ?>" class="img-fluid img-responsive rounded product-image" height="150px" width="220px"/>
              </div>
