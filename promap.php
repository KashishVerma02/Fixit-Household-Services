      ...
      <?php 
                     ...
                     // session_start();
                         $service = $_GET['id'];
                              ...
                              //echo $service; 
                             $sql = "SELECT serviceprovider.providerid,serviceprovider.img_text,serviceprovider.providername,serviceprovider.visit_charge,serviceprovider.description,serviceprovider.address, providerservices.serviceid,AVG(sp_rating.rating) 'ave rating' FROM serviceprovider,providerservices,sp_rating WHERE serviceprovider.providerid=providerservices.provider_id AND serviceprovider.providerid=sp_rating.providerid AND providerservices.serviceid=$service GROUP BY sp_rating.providerid ORDER BY serviceprovider.address  ";
...
//AS `distance`
//FROM `` HAVING `distance` < 25
//ORDER BY `distance`
//SELECT serviceprovider.providerid,serviceprovider.img_text,serviceprovider.providername,serviceprovider.visit_charge,serviceprovider.description, providerservices.serviceid,AVG(sp_rating.rating) 'ave rating' FROM serviceprovider,providerservices,sp_rating WHERE serviceprovider.providerid=providerservices.provider_id AND serviceprovider.providerid=sp_rating.providerid AND providerservices.serviceid=$service GROUP BY sp_rating.providerid ORDER BY serviceprovider.visit_charge
                               $result = $conn->query($sql);
                              while($row = mysqli_fetch_assoc($result)) {?>
          <div class="row p-2 bg-white border rounded"><?php //echo $row['provider_name'];?>
              ...
              <div class="col-md-6 mt-1">
                  ...
                  <p class="text-justify text-truncate para mb-0">  <?php echo $row['address']; ?> </p>
